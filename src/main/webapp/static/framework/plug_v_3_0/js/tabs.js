...
//                	}
            ...
            open:function(options,refresh){
                ...
                tab.find(".remove").click(function(){
                    ...
                    if(tab.hasClass(tabController.tab.tabActiveClass)){
                        ...
                        var tabid=$(_tab).attr("id");
                        
                        tabController.tab.select(_tab);
                        tabController.iframe.moveToTop($("iframe[data-id='"+tabid+"']"));
                    }
                    //移除将被关闭的选项卡
                ...
                remove:function(target){
                    if(typeof(target)=="object"){
                        ...
                        for(var i=0;i<this.tabsData.length;i++){
                            if(this.tabsData[i].id==id){//&&this.tabsData[i].close!=false
                            	 //移除元素
                                target.remove();
            ...
            iframe:{
                ...
                iframeHtml:"<iframe allowfullscreen='true'  webkitallowfullscreen='true'  mozallowfullscreen='true'></iframe>",
                style:{
                    width : "100%",
                    height : "100%",
                    position : "absolute",
