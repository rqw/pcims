...
import store from "../../store.js";
    const RSA={
         ...
         * @param options
         * @returns 返回encryptStr加密后的字符串
         */
        encrypt:function(encryptStr,options){
            console.log("exponent:"+store.state.profile.security.exponent);
            console.log("modulus:"+store.state.profile.security.modulus);
            var key = RSAUtils.getKeyPair(store.state.profile.security.exponent, '', store.state.profile.security.modulus);
            encryptStr = encodeURIComponent(encryptStr);
            encryptStr = RSAUtils.encryptedString(key, encryptStr);
            return encryptStr;
        }
    }
